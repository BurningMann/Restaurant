...
$(document).ready(function() {
	...
	$(".register").click(function() {
		...
		var input = $("<input>");
		$(forma).append(input);
		$(forma).attr("action", "call.php");
		$(forma).attr("method", "post");
		$(forma).attr("name", "call-form");
	...
	$(".register_eng").click(function() {
		...
		var p = $("<p/>")
		$(divForma).append(p);
		p.text("Indicate the number of guests, the desired time of visit, as well as any other additional information. We will call you back in the near future. Working hours: Mon-Sun - 7:00-23:00.");
		var forma = $("<form/>");
		$(divForma).append(forma);
		var input = $("<input>");
		$(forma).append(input);
		...
		$(input1).attr("type", "text");
		$(input1).attr("name", "phone");
		$(input1).attr("placeholder", "PHONE");
		$(input1).addClass("input");
		var textareas = $("<textarea>");
		$(forma).append(textareas);
		$(textareas).attr("type", "text");
		$(textareas).attr("name", "message");
		$(textareas).attr("placeholder", "BOOKING INFORMATION");
		$(textareas).addClass("textarea");
		var input2 = $("<input>");
		$(forma).append(input2);
		$(input2).attr("type", "submit");
		$(input2).attr("value", "REQUEST A CALL");
		$(input2).addClass("button");
...
function initMap() {
	...
	marker = new google.maps.Marker({
		...
		position: locations,
		title: 'Restaurant Burning Snake',
	});
